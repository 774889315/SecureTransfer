...
public class RSAUtil {
    ...
    public static byte[] publicEncrypt(byte[] data, RSAPublicKey publicKey) {
        try {
            ...
            return Base64.encodeBase64URLSafe(rsaSplitCodec(cipher, Cipher.ENCRYPT_MODE, data, publicKey.getModulus().bitLength()));
        } catch (Exception e) {
    ...
    public static byte[] privateEncrypt(byte[] data, RSAPrivateKey privateKey) {
        try {
            ...
            return Base64.encodeBase64URLSafe(rsaSplitCodec(cipher, Cipher.ENCRYPT_MODE, data, privateKey.getModulus().bitLength()));
        } catch (Exception e) {
            throw new RuntimeException("加密字符串[" + data + "]时遇到异常", e);
        }
    }
...
//测试
    ...
    String privateKey = keyMap.get("privateKey");
      ...
      String encodedData = RSAUtil.publicEncrypt(str, RSAUtil.getPublicKey(publicKey));
        ...
        String encodedData1 = RSAUtil.privateEncrypt(str, RSAUtil.getPrivateKey(privateKey));
        System.out.println("私钥加密密文：" + encodedData1);
        //用私钥对密文进行解密
        
        String decodedData1 = RSAUtil.publicDecrypt(encodedData1, RSAUtil.getPublicKey(publicKey));
